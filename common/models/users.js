 ...
 * @property {Number} settings.ttl Default time to live (in seconds) for the `AccessToken` created by `Users.login() / user.createAccessToken()`.
...
module.exports = function(Users) {
   ...
   *
   */
  Users.prototype.createAccessToken = function(ttl, options, cb) {
   ...
   *
   * ```js
   *    Users.login({username: 'foo', password: 'bar'}, function (err, token) {
    ...
    self.findOne({where: query}, function(err, user) {
      ...
      function tokenHandler(err, token) {
        ...
        if (Array.isArray(include) ? include.indexOf('user') !== -1 : include === 'user') {
          ...
          //     "AccessToken belongsTo Users" relation
      ...
      if (err) {
        debug('An error is reported from Users.findOne: %j', err);
        fn(defaultError);
      } else if (user) {
        user.hasPassword(credentials.password, function(err, isMatch) {
          if (err) {
            debug('An error is reported from Users.hasPassword: %j', err);
            fn(defaultError);
          } else if (isMatch) {
  ...
  *    });
  ...
  Users.logout = function(tokenId, fn) {
    fn = fn || utils.createPromiseCallback();
   ...
   * @promise
   */
  Users.changePassword = function(userId, oldPassword, newPassword, options, cb) {
    if (cb === undefined && typeof options === 'function') {
      cb = options;
      options = undefined;
    }
    ...
    this.findById(userId, options, (err, inst) => {
      ...
      if (!inst) {
        const err = new Error(`Users ${userId} not found`);
        Object.assign(err, {
          code: 'USER_NOT_FOUND',
          statusCode: 401,
        });
   ...
   * @promise
   */
  Users.prototype.changePassword = function(oldPassword, newPassword, options, cb) {
    if (cb === undefined && typeof options === 'function') {
      cb = options;
      options = undefined;
    }
   ...
   * @promise
   */
  Users.setPassword = function(userId, newPassword, options, cb) {
   ...
   * @promise
   */
  Users.prototype.setPassword = function(newPassword, options, cb) {
    assert(!!newPassword, 'newPassword is a required argument');
   ...
   *
   * ```js
   * var verifyOptions = MyUsers.getVerifyOptions();
   ...
   *
   * ```js
   * customUsersModel.getVerifyOptions = function() {
   *   const base = MyUsers.base.getVerifyOptions();
  ...
  Users.prototype.verify = function(verifyOptions, options, cb) {
    if (cb === undefined && typeof options === 'function') {
      cb = options;
      options = undefined;
    }
    ...
    verifyOptions.generateVerificationToken = verifyOptions.generateVerificationToken ||
      Users.generateVerificationToken;
    ...
    if (tokenGenerator.length == 3) {
      tokenGenerator(user, options, addTokenToUsersAndSave);
    } else {
      tokenGenerator(user, addTokenToUsersAndSave);
    }

    function addTokenToUsersAndSave(err, token) {
      if (err) return cb(err);
   ...
   *
   * @param {object} user The Users this token is being generated for.
   * @param {object} options remote context options.
   * @param {Function} cb The generator must pass back the new token with this function call.
   */
  Users.generateVerificationToken = function(user, options, cb) {
   ...
   * @promise
   */
  Users.confirm = function(uid, token, redirect, fn) {
    fn = fn || utils.createPromiseCallback();
      ...
      if (err) {
        ...
        if (user && user.verificationToken === token) {
          ...
          if (user) {
            ...
            err = new Error(g.f('Users not found: %s', uid));
            err.statusCode = 404;
  ...
  Users.resetPassword = function(options, cb) {
    cb = cb || utils.createPromiseCallback();
    var UsersModel = this;
    var ttl = UsersModel.settings.resetPasswordTokenTTL || DEFAULT_RESET_PW_TTL;
    options = options || {};
    ...
    try {
      if (options.password) {
        UsersModel.validatePassword(options.password);
      }
    } catch (err) {
      return cb(err);
    }
    ...
    UsersModel.findOne({where: where}, function(err, user) {
      if (err) {
        return cb(err);
      }
      if (!user) {
      ...
      if (UsersModel.settings.emailVerificationRequired && !user.emailVerified) {
      ...
      if (UsersModel.settings.restrictResetPasswordTokenScope) {
        const tokenData = {
          ttl: ttl,
      ...
      function onTokenCreated(err, accessToken) {
        ...
        cb();
        UsersModel.emit('resetPasswordRequest', {
          email: options.email,
  ...
  Users.validatePassword = function(plain) {
    var err;
  ...
  Users._invalidateAccessTokensOfUsers = function(userIds, options, cb) {
    ...
    // between AccessToken and Users
    var relatedUsers = AccessToken.relations.user;
    var isRelationPolymorphic = relatedUsers && relatedUsers.polymorphic &&
      !relatedUsers.modelTo;
  ...
  Users.setup = function() {
    ...
    UsersModel.setter.email = function(value) {
      if (!UsersModel.settings.caseSensitiveEmail && typeof value === 'string') {
    ...
    UsersModel.setter.password = function(plain) {
    ...
    UsersModel.beforeRemote('create', function(ctx, user, next) {
      ...
      'login',
      {
        ...
        returns: {
          ...
          description:
            ...
            '  - `user` - `U+007BUsersU+007D` - Data of the currently logged in user. ' +
            '{{(`include=user`)}}\n\n'),
        },
        http: {verb: 'post'},
      }
    ...
    UsersModel.remoteMethod(
      'changePassword',
      {
        ...
        accepts: [
          {arg: 'id', type: 'any', http: getUsersIdFromRequestContext},
    ...
    const setPasswordScopes = UsersModel.settings.restrictResetPasswordTokenScope ?
    ...
    function getUsersIdFromRequestContext(ctx) {
      const token = ctx.req.accessToken;
      if (!token) return;

      const hasPrincipalType = 'principalType' in token;
      if (hasPrincipalType && token.principalType !== UsersModel.modelName) {
    ...
    UsersModel.afterRemote('confirm', function(ctx, inst, next) {
    ...
    // default models
    assert(loopback.Email, 'Email model must be defined before Users model');
    UsersModel.email = loopback.Email;

    assert(loopback.AccessToken, 'AccessToken model must be defined before Users model');
    UsersModel.accessToken = loopback.AccessToken;

    UsersModel.validate('email', emailValidator, {
      message: g.f('Must provide a valid email'),
    });

    // Realm users validation
    if (UsersModel.settings.realmRequired && UsersModel.settings.realmDelimiter) {
      UsersModel.validatesUniquenessOf('email', {
        message: 'Email already exists',
        scopedTo: ['realm'],
      });
      UsersModel.validatesUniquenessOf('username', {
        message: 'Users already exists',
        scopedTo: ['realm'],
      });
    } else {
      // Regular(Non-realm) users validation
      UsersModel.validatesUniquenessOf('email', {message: 'Email already exists'});
      UsersModel.validatesUniquenessOf('username', {message: 'Users already exists'});
    }
  ...
  Users.observe('access', function normalizeEmailCase(ctx, next) {
  ...
  Users.observe('before save', function rejectInsecurePasswordChange(ctx, next) {
    const UsersModel = ctx.Model;
    if (!UsersModel.settings.rejectPasswordChangesViaPatchOrReplace) {
      // In legacy password flow, any DAO method can change the password
      return next();
    }

    if (ctx.isNewInstance) {
      // The password can be always set when creating a new Users instance
      return next();
    }
  ...
  Users.observe('before save', function prepareForTokenInvalidation(ctx, next) {
    ...
    ctx.Model.find({where: where}, ctx.options, function(err, userInstances) {
      if (err) return next(err);
      ctx.hookState.originalUsersData = userInstances.map(function(u) {
        var user = {};
      ...
      if (ctx.instance) {
        ...
        if (ctx.hookState.originalUsersData.length > 0) {
          emailChanged = ctx.instance.email !== ctx.hookState.originalUsersData[0].email;
        } else {
          emailChanged = true;
        }
        ...
        emailChanged = ctx.hookState.originalUsersData.some(function(data) {
  ...
  Users.observe('after save', function invalidateOtherTokens(ctx, next) {
    if (!ctx.instance && !ctx.data) return next();
    if (!ctx.hookState.originalUsersData) return next();
    ...
    var userIdsToExpire = ctx.hookState.originalUsersData.filter(function(u) {
